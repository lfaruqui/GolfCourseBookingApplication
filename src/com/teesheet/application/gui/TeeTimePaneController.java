...
public class TeeTimePaneController {
	...
	ExpandedReservationController rc;
	
	EmptyTeeTimeTitleController ec;
...
//			pane.setExpanded(false);
	...
	private HBox getNewGraphic() {
		FxmlLoader graphicLoader = new FxmlLoader("EmptyTeeTimeTitle.fxml");
		
		HBox graphic = (HBox) graphicLoader.loadPage();
		
		ec = graphicLoader.getLoader().getController();
		
		
		
		ec.setDate(date);
		ec.setTeeTime(teeTime);
		ec.setParentController(this);
		
		return graphic;
	}
	
	public void setTeeTimeDate(String date) {
		this.date = date;
		rc.setDate(date);
	}
