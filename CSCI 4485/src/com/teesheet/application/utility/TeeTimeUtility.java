		...
		try {
			...
			while (queryResult.next()) {
				...
				if (expandedTime.equals(time)) {
					...
					TeeTime t = new TeeTime(date, time, blocked, memberName, memberPres, memberCart, player1Name,
							p1Cart, p1Ref, player2Name, p2Cart, p2Ref, player3Name, p3Cart, p3Ref, player4Name, p4Cart,
							p4Ref, caddie, creator);
					JsonString j = new JsonString();

					ObjectMapper map = new ObjectMapper();

					String json = map.writerWithDefaultPrettyPrinter().writeValueAsString(t);

					j.setValue(json);
					jsonArray.addValue(j);

				} else {
					TeeTime t = new TeeTime(date, time, blocked, memberName, player1Name, player2Name, player3Name,
							player4Name, caddie);
					JsonString j = new JsonString();
...
//					System.out.println(j.toFormattedString());
					jsonArray.addValue(j);
				}
				i++;
			}
	...
	public static void main(String[] args) {
		JsonArray js = TeeTimeUtility.loadTeeSheet("2022-04-04", "");

		System.out.println(js.toFormattedString());
